...
function getBestVoice(lang) {
  ...
  const all = voices.filter(v => v.lang.startsWith(prefix));
  if (!all.length) return voices[0] || null;
  // Prefer local (system) voices - better quality than remote
  const local = all.filter(v => v.localService);
  const pool = local.length ? local : all;
  if (lang === "es") {
    return pool.find(v => v.lang === "es-ES") ||
           pool.find(v => v.lang === "es-MX") ||
           pool[0];
  }
...
function populateVoiceSelect(lang) {
  ...
  const voices = window.speechSynthesis.getVoices();
  const prefix = lang === "es" ? "es" : "en";
  const relevant = voices.filter(v => v.lang.startsWith(prefix));
  const all = relevant.length ? relevant : voices;
  ...
  // Restore previously selected or pick best
  if (selectedVoiceName) {
    const match = all.find(v => v.name === selectedVoiceName);
    if (match) els.voiceSelect.value = match.name;
  }
}

function trySpeak(textObj) {
  const lang = detectLangForSpeech(textObj.body);
  populateVoiceSelect(lang);
  const voices = window.speechSynthesis.getVoices();
  const voice = selectedVoiceName
    ? voices.find(v => v.name === selectedVoiceName) || getBestVoice(lang)
    : getBestVoice(lang);
  const utterance = new SpeechSynthesisUtterance(textObj.body);
  if (voice) { utterance.voice = voice; utterance.lang = voice.lang; }
  else { utterance.lang = lang === "es" ? "es-ES" : "en-GB"; }
  utterance.rate = 0.88;
  utterance.pitch = 1.0;
  utterance.volume = 1.0;
  utterance.onstart = () => { isSpeaking = true; els.speechBtn.textContent = "■ Stop"; };
  utterance.onend = () => { isSpeaking = false; currentUtterance = null; els.speechBtn.textContent = "▶ Play"; };
  utterance.onerror = () => { isSpeaking = false; currentUtterance = null; els.speechBtn.textContent = "▶ Play"; };
...
function initSpeechButton() {
  ...
  readerActions.prepend(voiceSelect);
  readerActions.prepend(btn);
