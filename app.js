...
function getVoiceForText(text) {
  ...
  const looksSpanish = /[áéíóúüñ¿¡]/i.test(text) ||
    (text.match(/\b(el|la|los|las|de|que|en|un|una|por|con|para|del)\b/gi) || []).length > 3;
...
function toggleSpeech() {
  ...
  const text = getAllTexts().find(t => t.id === activeTextId);
  if (!text) return;

  const utterance = new SpeechSynthesisUtterance(text.body);
  const voice = getVoiceForText(text.body);
...
// Stop speech when switching texts
const _origRenderApp = renderApp;
function renderApp() {
  if (isSpeaking) {
    window.speechSynthesis.cancel();
    isSpeaking = false;
    if (els.speechBtn) els.speechBtn.textContent = "▶ Play";
  }
  if (isTranslated) {
    isTranslated = false;
    if (els.translateBtn) {
